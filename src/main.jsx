// src/main.jsx
import { StrictMode, useState, useEffect } from 'react' // <-- 1. TAMBAHKAN 'useEffect'
...
  };
  ...
  // Pindahkan ini ke atas agar bisa dipakai di useEffect
...
  };
  ...
  useEffect(() => {
    // Hanya jalankan setelah splash screen selesai
    if (!showSplash) {
      const urlParams = new URLSearchParams(window.location.search);
      const recipeId = urlParams.get('recipe');
      const category = urlParams.get('category');
      
      // Jika ada parameter recipe di URL
      if (recipeId) {
        // Panggil fungsi yang sama seolah-olah resep diklik
  ...
  }, [showSplash]); // Hook ini akan berjalan 1x setelah showSplash menjadi false
  // --- BATAS PENAMBAHAN ---
...
  };
  ...
  // (Fungsi ini sudah dipindah ke atas)
  // const handleRecipeClick = (recipeId, category) => { ... };
